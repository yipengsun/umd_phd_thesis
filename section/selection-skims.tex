...
The signal and control skims (sub-samples) are separated by inspecting if the
\DXmu pair is well-separated from all other well-reconstructed (long) charged
tracks in the event (isolated);
if the pair is isolated,
