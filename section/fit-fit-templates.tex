...
the $\Bsb \rightarrow D_{s1}^{'+}\mu$ and
$\Bsb \rightarrow D_{s2}^{*+}\mu$ templates.
Both of the \DststS mesons decay to a \Dstarp\Kz pair which contributes to
both fit channels.
The yields in the \Dz channel are constrained separately:
\begin{equation}
    \fitNDmu \times \left\{
        \fitNDststDz \times \mathcal{B}_{\DststS} \times \fitfsfd \times
        N_{\DststS}
    \right\}
\end{equation}
