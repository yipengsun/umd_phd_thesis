...
%%%%
\paragraph{Muonic}
The normalization factor of a \Dstst ``species'' $S$ in the \Dz channel is given by
\begin{equation}
    \fitNDmu \times \left\{
         \times \fitNDststDz \times \fitBFDDS
    \right\}
\end{equation}
where the allowed species are $D_0^*, D'_1, D_1, D_1\pi\pi, D_2^*$, and
\begin{itemize}
    \item $\fitNDststDz \equiv \frac{10^{-3}}{\mathcal{B}(\Bm \rightarrow \Dz\mun\neumb)}$
        is a convenience factor so that the floating \fitBFDDS can be
        interpreted as the branching fraction of $S$ multiplied by its selection
        efficiency.
\end{itemize}
