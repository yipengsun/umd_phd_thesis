...
namely combinatorial background and \muon misID templates,
are discussed first in \cref{ref:fit:tmpl:comb,ref:fit:tmpl:misid}.
...
The generation procedure is documented below.
...
\label{ref:fit:tmpl:comb:dst-comb}
...
\subsubsection{Unfolding of fake muon control sample}
\label{sec:data-driven-templates:mu-misid:unfold-fake-mu}

% TODO: Steal a lot from run 1 ANA, sec 8.5. Should we cite that?
The key input to build the \muon misID template is a fake \muon control sample
that passes the \texttt{b2D0MuXFakeB2DMuForTauMuLine} stripping line.
This line contains a normally reconstructed $D^{0}$
paired with a track $t$ that \emph{fails} muon identification but passes all other
nominal muon cuts such as acceptance.

The misID template,
representing non-\muon tracks that pass the \muon identification requirements in
the signal region,
is generated from the control sample above with an unfolding technique.
This technique, first conceived in \cite{LHCb-ANA-2016-059},
consists of the following steps:

\begin{enumerate}
    \item Tag the fake \muon sample into $\pi$, $K$, $p$,
        $e$, and ghost species with the selection requirements listed in
        \cref{tab:selection-for-tagged-species}.
        Denote tagged species with a hat:
        $\hat{t} \in \{\hat{\pi}, \hat{K}, \hat{p}, \hat{e}, \hat{g}\}$.

    \item Obtain the efficiency for an event in the fake \muon sample that contains a true track species $t$ to be classified as a
        another (or the same) track species $\hat{t}'$: $\misEff[t_\text{fake}]{\hat{t}_\text{fake}'}$.
        These efficiencies are obtained from \pidcalib for $\pi, K, p, e$ and from MC for $g$.

    \item Given the measured yields in the fake \muon sample $n_{\hat{t}_\text{fake}'}$ and response matrix
        $M_{\hat{t}_\text{fake}', t_\text{fake}^{\phantom{}}} = \misEff[t_\text{fake}]{\hat{t}_\text{fake}'}$,
        % NOTE: The subscript of M is indeed tag, true!!! NOT true, tag!!!
        unfold the true yields $n_{t_\text{fake}}$.
        ...
        $\hat{t}'_\text{fake}$ being a true $t_\text{fake}$: $\misEff[\hat{t}_\text{fake}']{t_\text{fake}}$.

      \item Find $\misEff[\hat{t}'_\text{fake}]{\hat{\mu}_\text{sig}}$,
        the transfer factor from the fake \muon to the signal sample,
        based on
        \begin{itemize}
          \item the efficiencies obtained in the step above,
          \item the probability of a true track $t$ that satisfies \muon
              acceptance cuts to also satisfy the \muon PID requirements in the
              signal sample:
            $\misEff[t_\text{acc}]{\hat{\mu}_\text{sig}}$
          \item and the probability of $t_\text{acc}$ to fail \muon PID:
            $\misEff[t_\text{acc}]{t_\text{fake}}$.
        \end{itemize}

        \begin{equation}
            \misEff[\hat{t}'_\text{fake}]{\hat{\mu}_\text{sig}} =
                \sum_{t}
                \frac{\misEff[\hat{t}'_\text{fake}]{t_\text{fake}}}{\misEff[t_\text{acc}]{t_\text{fake}}}
                \misEff[t_\text{acc}]{\hat{\mu}_\text{sig}}
        \end{equation}

        These probabilities are also found from \pidcalib.

    \item Apply $\misEff[\hat{t}'_\text{fake}]{\hat{\mu}_\text{sig}}$ as a weight for each
        tagged species $\hat{t}'_\text{fake}$.
        The weighted tagged species represent the misID contributions of the
        species to the signal sample.
\end{enumerate}

Comparisons between some raw and unfolded variables of the misID sample are
shown in \cref{fig:unfolding-binning-vars}.
...
\begin{figure}[htb]
    ...
    \caption[misID tagged vs. unfolded.]{
        ...
        Top are the raw tagged yields in the fake \muon control sample,
        bottom are the unfolded true yields.

        The samples displayed here are 2016 $D^0$ misID control samples
        $B^- \rightarrow D^0 t^-$ passing nominal global cuts.
    }
