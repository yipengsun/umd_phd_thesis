...
The \Dstarp\muon template contribute to both fit channels due to feed down,
...
\begin{equation}
    ...
    \right\}
\end{equation}
where $\epsilon_{\Dstar}^\text{isolation}$ is the relative isolation efficiency
between \Dstarp and \Dstarz samples
\begin{equation}
    \epsilon_{\Dstar}^\text{isolation} = \frac{
        \epsilon^\text{isolation}(\Bzb \rightarrow \Dstarp\mun\neumb)
    }{
        \epsilon^\text{isolation}(\Bm \rightarrow \Dstarz\mun\neumb)
    } \times
    \frac{
...
\end{equation}
obtained with MC samples;
``normfd'' is the branching fraction ratio between the \Dstarz\muon and \Dz\muon
samples \emph{with isolation efficiency taken into account}:
\begin{equation}
    \text{normfd} =
    \epsilon^\text{isolation}(\Bm \rightarrow \Dstarz\mun\neumb) \times
    \frac{
...
Its yield is constrained by $N_{D\mu} \times \text{normfd}$.
