...
The misID templates are generated by unfolding different contributions from the
...
\begin{enumerate}
    \item Remove the misID contribution from the RS sample.
    \item Remove the misID contribution from the WS $\mu$ sample.
    \item Perform a fit in the upper-sideband region of the RS sample to
        determine the contribution from \DstComb,
        with procedure identical to that in \cref{dst-comb}.
        Then remove the fitted \DstComb.
    \item Fit and remove \DstComb\ from the WS $\mu$ sample.
