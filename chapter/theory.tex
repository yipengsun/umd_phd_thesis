...
\begin{align}
    ...
    S_1(w) &= h_+(w) - \frac{m_B + m_D}{m_B - m_D}\frac{w-1}{w+1} h_-(w)
\end{align}
and an expression for $V_1(w)$ is obtained by first analytically continue $w$
outside the physical

The CLN parameterization  utilizes a dispersion relation
based on first principles,
namely the analyticity of the two-point function of the local current and
the positivity of the corresponding spectral functions \cite{Caprini_1998},
which provides model-independent constraints to the FFs and
rewrite hadronic current matrix elements as a series expansion of $z$,
where $z$ is obtained via a conformal mapping $q \rightarrow z$ after
analytically continue $q$ outside the physical region to the full complex plane.
