...
3 spin-1 particles $W^1, W^2, W^3$ are added to the theory in addition
in addition to $B$ which is the only generator of $U(1)$,
...
\begin{equation}
    \partial_\mu - i[g_2 W_\mu^a T^a_{SU(2)} + g_1 B_\mu Y_{U(1)}]
\end{equation}
...
which is inconsistent with the experimental results which require the
force carrying bosons of $SU(2)$ to be massive\footnote{
    If $W^1, W^2, W^3$ are all massless, the weak interactions would behave like
    electromagnetism, which they don't.
}.

fixed by Higgs mechanism

also need to discriminate between left-handed leptons and right-handed leptons,
as the weak interaction is experimentally shown to only couple to the
former (left-handed).
