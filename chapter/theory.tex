...
The leptonic current corresponds to $S_1$ is helicity suppressed and,
as a result,
...
The use of CLN parameterization has received the following criticism
\cite{LHCb-ANA-2020-056}:

\begin{itemize}
    \item The slope and curvature,
        that is, the coefficients of the $z$ and $z^2$ terms
        in \cref{eqn:cln-dst-bad-correlation},
        of the Isgur-Wise functions
        share the same parameter $\rho$,
        making the two too closely cross-constrained by the model.

    \item In \cref{eqn:cln-dst-bad-coeff-1,eqn:cln-dst-bad-coeff-2,eqn:cln-dst-bad-coeff-3},
        the numerical coefficients are $w$-dependent and
        are estimated at a \emph{fixed} $w = 1$.
        The change of these coefficients at $w \neq 1$ are not taken into
        account.
\end{itemize}
