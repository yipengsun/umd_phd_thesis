...
$S_1 = (\psi_L)^\dagger \phi_R, S_2 = (\phi_R)^\dagger(\psi_L)$;
the first order partial derivative terms
$S_3 = (\psi_L)^\dagger \partial_\mu \bar{\sigma}^\mu (\psi_L),
S_4 = (\phi_R)^\dagger \partial_\mu {\sigma}^\mu (\phi_R)$.
This shows that both left- and right-handed particles are needed for Lorentz
invariance,
which motivates a 4-component Dirac spinor
$\Psi = \bigl(\begin{smallmatrix} \psi_L \\ \phi_R \end{smallmatrix}\bigr)$.
%%%%
...
A \emph{local} gauge transformation can be implemented by
promoting $\alpha$ from a number to a function $\alpha(x^\mu)$,
in which case the Lagrangian is no longer invariant due to the derivative
$\partial_\mu$.
A vector field operator $A_\mu$ can be introduced to restore the symmetry,
which satisfies the gauge transformation
$A_\mu \rightarrow A_\mu + \frac{1}{g} \partial_\mu \alpha(x^\mu)$,
by replacing the standard derivative $\partial_\mu$ with the following:
...
However, it is ``unstable'' as the equation of motion of $A_\mu$
...
where $F^{\mu\nu} = \partial^\mu A^\nu - \partial^\nu A^\mu$.
Putting all together, one obtains the Lagrangian for QED:

\begin{equation}
    \mathcal{L} = \bar{\Psi} (i\gamma^\mu\fsl{D}_\mu - m) \Psi - \frac{1}{4}F_{\mu\nu}F^{\mu\nu}
\end{equation}
