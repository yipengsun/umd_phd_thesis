...
Write the complex scalar as a $SU(2)$ doublet
...
which transform as
$\Psi_L \rightarrow \Psi_L e^{i \alpha_i \frac{\sigma^i}{2}}$;
...
Parity violation provides a different view point on why these are forbidden:
the naive mass terms contain \emph{only} combinations of left-handed particles
and right-handed particles, such as $\bar{\Psi}_L \psi_R$,
due to requirement on Lorentz invariance.
...
With the electroweak theory developed in \cref{ew-th}, and accepting the
experimental observation that leptons do not interact strongly
