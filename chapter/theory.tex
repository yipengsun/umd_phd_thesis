...
\section{Semileptonic $B \rightarrow D l^- \neulb$ decays in the SM}

Semileptonic $B \rightarrow D l^- \neulb$ decays,
where $D$ stands for a generic charm meson and $l$ a generic lepton,
play an central role in this analysis as signal, normalization,
...
determine generation of the Monte-Carlo simulation samples that provide
estimations of the fit variables for each of such decays.
This section describes the formalism of such a modelling.

The parton level process involved in the $B \rightarrow D l \neulb$ decays is
$b \rightarrow c l \neulb$,
...
\end{equation}
in which the leptonic and hadronic currents factorize because lepton
are color singlets and do not interact strongly.
...
\begin{align}
    ...
    \bra{l(p_l, \lambda_l) \neulb(p_\neulb)} \bar{l} \gamma^\mu P_L \neul
    \ket{0} \\
    H^{\lambda_D}_{\lambda_W}(q^2)
    & =
    \epsilon_\mu(\lambda_W)
    \bra{D(p_D, \lambda_D) \neulb(p_\neulb)} \bar{c} \gamma^\mu P_L b
    \ket{\bar{B}(p_B)}
\end{align}
where $p_i, \lambda_i$ refer to the four-momenta and helicities of the particle
$i$;
$\theta_l$ refers to helicity angle in the $l$ rest frame.
...
\subsection{Form factors in $B \rightarrow D^{(*)}\ell\neulb$ decays}
