...
\cite{HAGIWARA1989569}, and are copied verbatim for convenience:
...
Hence, these currents are not computable in an analytical way and are expressed
in terms of form factors (FFs),
which are a function of \qSq, instead.
These FFs are estimated with quark models,
heavy quark effective theory, or lattice QCD;
the latter two are sometimes used in conjunction.
...
produced at rest in the $B$ rest frame
(referred to as ``zero recoil'' due to the fact that the $D$ meson is
\emph{not recoiling} in the $B$ rest frame),
and are extrapolated to the full $q^2$ range with experimental inputs.

While not analytically calculable,
the hadronic currents are constrained by conservation of angular momentum:
the $D,W$ system must have the same angular momentum of the spin 0 pseudoscalar
$B$ meson,
which implies that for $D^0$ meson, also a pseudoscalar,
the only non-zero hadronic amplitude are $H^0_0, H^0_s$.
For $D^*$ meson which is a spin 1 vector,
the amplitudes $H^+_+$ and $H^-_-$ are allowed in addition.

Still, due to conservation of helicity between the


\subsection{Form factors in $B \rightarrow D^{(*)}\ellm\neulb$ decays}
\label{ref:theory:ff-d0-dst}

the d and dst ffs are both modeling w/ BGL which are good for reasons.
