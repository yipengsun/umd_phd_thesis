...
\begin{itemize}
    ...
    \item Tracker Turicensis:
        ...
        configuration\footnote{
            This will be explained in \cref{part:ut}.
        }.
...
bandwidth and storage limitations\footnote{
    The LHC bunch crossing frequency is 40~MHz.
    It is estimated in \cite{The_LHCb_Collaboration_2008} that to readout all
    visible events from the detector, a bandwidth of $\tilde 10$~MHz is needed.
}.
The hardware trigger, referred as Level-0 (L0) triggers, reduces the bandwidth
from 40~MHz to 1~MHz, at which rate events are readout and further processed by
software high-level triggers (HLT1 and HLT2) to further reduce the rate down to
about 2~kHz before writing to disk for offline analysis
\cite{The_LHCb_Collaboration_2008}.
...
3 with more details in \cref{part:ut}.
