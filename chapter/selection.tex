...
%%%%
The HLT1 paths are updated to the inclusive triggers,
which select events containing a particle whose decay vertex is displaced
from the $pp$ vertex,
for LHCb run 2,
...
further tighten the kinematic and vertex quality requirements on the selected
candidates.

Overall, the online selection for \Dz channel requires a \muon candidate to form
a well-defined vertex displaced from the $pp$ vertex with a
