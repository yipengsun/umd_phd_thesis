...
which uses the \dim protocol for bi-directional communication to the SCADA
system.
...
As discussed in \cref{ref:ut:online},
The SOL40s provide the control of FEs, whereas the TELL40 provide data
readout functionality.
Both are implemented by the same underlying hardware, the PCIe40,
...
%   SMI (state man interface) manager registered to the DIM DNS for FSM
...
% The DCBs and Hybrids are controlled by SOL40 directly, the LVRs are
...
% ELMB seems to be a monitoring board that has ADCs. What kinds of
...
%   (Stave -> BP -> Telemetry (innermost, 2 thermistors of hybrid, 1 to ELMB,
%   outermost 1 per hybrid always to ELMB, and 4 sensors on CO2 pipe))
%   (Humidity sensors in the detector box directly routed to ELMB)
%   ELMB always on (DCB are not)

% How many TELL40s and SOL40s are planned to be used currently?
%   7 (3+3 for DCB, 1 for TCB) SOL40s,
%   All 8 SOL40 in a single server
%
%   54 TELL40s (PCIe40) per side (x2)
