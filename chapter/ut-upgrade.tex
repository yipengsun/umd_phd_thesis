...
\begin{tikzpicture}
    ...
    \begin{scope}[x=(main.south east),y=(main.north west)]
        \node[text=cyan] (A) at (0.7,0.92) {\textbf{PEPI}};
        \node (B) at (0.5,0.73) {};
        \draw[->, thick, draw=cyan, to path={-| (\tikztotarget)}] (A) edge (B);

        \node[text=cyan] (C) at (0.3,0.14) {\textbf{Detector box}};
        \node (D) at (0.5,0.52) {};
        \draw[->, thick, draw=cyan, to path={-| (\tikztotarget)}] (C) edge (D);

        \node[text=cyan] (E) at (0.7,0.8) {\textbf{Service bay}};
        \node (F) at (0.82,0.74) {};
        \draw[->, thick, draw=cyan, to path={-| (\tikztotarget)}] (E) edge (F);
    \end{scope}
\end{tikzpicture}
