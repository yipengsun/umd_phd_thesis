...
A rotated sensor provides positional measurement with another strip that is not
co-linear with the first one, forming a ``X'' pattern with the first strips and
the $x$-$y$ two-dimensional position is determined as the intersection between
the two.
Such process is illustrated in \cref{fig:strip-sensor-measurement}.
...
\begin{figure}[ht]
    ...
    \caption{
        Illustration of measuring track hit position with two silicon-strip
        sensors.
        The second sensor is required to rotate by an angle.
    }
