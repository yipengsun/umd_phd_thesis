...
\begin{itemize}
    \item \textbf{SODIN}:
        ...
        distributes it to every single detector components
        (from FE to back-end readers).
        It also issue Timing and Fast Control (TFC) commands to the front-end to
        regulate data flow.

    \item \textbf{SOL40}:
        Fan out TFC commands to the reader and FE,
        while providing an interface between the FE and the control system of
        the experiment.
...
It is also possible load a firmware combining all three components with reduced
read bandwidth, in which case the PCIe40 forms the foundation of the
MiniDAQ system used by the collaborating institutions to develop FE
\cite{GranadoCardoso:2702137}.

The remaining section first provide an overview of the fast
(\cref{online-tfc}) and slow (\cref{online-dim}) control ``protocols'',
before jumping into an introduction on the MiniDAQ system
(\cref{online-minidaq}).
