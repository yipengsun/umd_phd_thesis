...
With \nanoDAQ, a DCB can be configured \emph{programmatically}, for example with
the following \smalltt{bash} script, a DCB is configured by just running a
single command:
...
\nanoDAQ also comes with an e-link phase-adjust script, which performs the
...
\begin{enumerate}
    ...
    \item Loop over all possible DCB e-link phases, and save the readout for
        ...
        if all readout Bytes are a phase shift of the fixed pattern
        \smalltt{C4}.
        If not, and if only a small percentage of the Bytes are wrong, the phase
        is marked in \textcolor{orange}{\bfseries yellow};
        phases with a large percentage of error are marked with a
        \textcolor{red}{\bfseries red ``X''}.

    \item A stable phase,
        marked in \textcolor{olive}{\bfseries green},
        is determined for each e-link by picking
        the central phase of consecutive stable phase.
