...
Hence, to reach target statistical uncertainties for MC,
...
The L0 hadron is triggered when the sum of the transverse energy $E_T$ deposited
...
system\footnote{
    The association algorithm can be looked up at the LHCb reconstruction
    software, located at
    \url{https://gitlab.cern.ch/lhcb/Analysis/-/blob/run2-patches/Phys/DecayTreeTupleTrigger/src/TupleToolL0Calo.cpp}.
    See the \lstinline{getAssociatedCluster} method.
},
the highest $E_T$ is above the triggering threshold.

Tracker-only MC does not have $E_T$ simulated, but it does record the
tracker-predicted position where a track hits the calorimeters.
In other words, though the triggering variable $E_T$ is not accessible,
...
the relevant variables, listed in \cref{tab:l0-hadron-emu-vars}, and the trigger
decision to train some regressor,
which can in turn be used to predict trigger decisions based on the training
variables alone, without requiring emulating $E_T$ directly.
Below the emulation procedure, inspired by \cite{LHCb-INT-2019-025}, is described:

\begin{enumerate}
    \item The \lstinline{TupleToolL0Calo} is configured to save HCAL
        information.
    \item A BDT-based regressor, \xgboost, is used.
    \item The training variables, listed in \cref{tab:l0-hadron-emu-vars}, and
        the trigger decision from a FullSim MC\footnote{
            MC decay ID: \lstinline{11574021}.
        } of decay
        $\Bz \rightarrow \Dstarp \mu \nu$, a normalization mode,
        is used to train \xgboost.
...
whereas the reconstruction contains only 3 tracks (\Km, \pip, and \mun, as \Dz
and \Bm are reconstructed)!
...
Unfortunately, there is a correlation between the two triggers introduced by
the fact that \bbbar are produced in pairs, making kinematics of the signal \B
meson and the background \Bbar meson, part of rest of event, correlated.
