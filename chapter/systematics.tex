...
\begin{table}[!htb]
    ...
    \begin{tabular}{r | c | c }
        \toprule
        {\bf Source} & {\bf $\sigma(\RDst) \times 10^2$} &
                       {\bf $\sigma(\RD)   \times 10^2$} \\
        \midrule
        %%%%
        $B \rightarrow D^{(*)}\ell\neulb$ form factors &
        0.53 (0.58) & 0.96 (2.37) \\
        ...
        Control sample shape parameters\parnote{
            ...
            This uncertainty is not part of the uncertainties reported by the
            fit.
        } &
        (0.87) & (4.36) \\
        %%%%
        ...
        0.21 (0.17) & 0.23 (0.30) \\
        %%%%
        $B \rightarrow D^{(*)} D^{**}_s (\rightarrow \taum\neutb) X$ bkg &
        0.37 (0.25) & 0.59 (1.21) \\
        %%%%
...
% When taking the quadrature difference, the systematic can be evaluated w/
% the following Python code:
%
%   from math import sqrt
%
%   def sysRD(sigmaPAlt, sigmaMAlt):
%       sigmaP = 5.986378
%       sigmaM = 5.982923
%       return sqrt(abs(sigmaP * sigmaM - sigmaPAlt * sigmaMAlt))
%
%   def sysRDst(sigmaPAlt, sigmaMAlt):
%       sigmaP = 1.940159
%       sigmaM = 1.939449
%       return sqrt(abs(sigmaP * sigmaM - sigmaPAlt * sigmaMAlt))
