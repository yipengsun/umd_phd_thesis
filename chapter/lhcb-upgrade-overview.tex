...
the LHCb detector readout was upgraded during 2019--2023 to achieve a readout
rate of 40~MHz,
...
without any hardware trigger,
making LHCb the first (hardware) trigger-less detector in a proton collider.
...
the main differences between the run 2 and run 3 trigger schemes are:
...
The need for a higher readout rate at 40~MHz is due to the saturation of the
hadronic L0 trigger:
this trigger requires the maximum energy $E_T$ deposited in the
...
With a 40~MHz readout rate at 13~TeV and a pile-up of 5.2
(corresponding to the run 3 conditions),
...
However,
the write throughput is limited to 10~GB/s
due to constraints from the LAN connection speed between the LHCb on-site online
system (responsible for data acquisition) and the CERN's storage center (Tier0)
in Meryin\footnote{
    A small city near Geneva.
}, Switzerland.
According to Ref.~\cite{CERN-LHCC-2014-016},
there are more than 20 pairs of fibers,
each capable of transmitting data at 1~GB/s,
which, in theory, can provide a bandwidth greater than 20~GB/s.
...
mainly due to the fact that with run 3 conditions, a significant portion of
collisions contains genuine $B$ mesons.
Instead, it is preferable to employ a trigger strategy that categorize different
types of signals and then apply selection cuts with as much information from an
event as possible, as early as possible
\cite{Albrecht_2014}.
...
% more about bandwidth comparison between LHCb and CMS/ATLAS:
% https://indico.fnal.gov/event/46746/contributions/210789/attachments/141129/177617/HLT1_CPAD_032021.pdf
...
Still based on silicon-strip technology,
the sensors in UT has a pitch of 180~\mum in the peripheral,
and a smaller pitch of 95~\mum compared to that of TT (183~\mum) around the beam
pipe.
The sensors also sit closer to the beam.
...
The Scintillating Fiber tracker (SciFi),
replacing the T1--T3 stations (both IT and OT),
