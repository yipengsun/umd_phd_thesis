...
the HLT software triggers perform full event reconstruction at the first stage,
instead of partial reconstruction;
...
The third point, 10~GB/s write throughput,
comes into play.
This number is limited by the LAN connection speed from the LHCb online system
(responsible for data acquisition) to CERN's storage center (Tier0) in Meryin.
...
%%%%
At 13~TeV with a pile-up of 5.2 (similar to run 3 condition), with
moderate \pt and vertex requirements
(similar to inclusive HLT1 triggers in run 2),
assuming an event size of 100~kB\footnote{
    The original text uses kb (kilo-bit), which does not add up to 80~GB/s for
    a 800~kHz rate.
    I suspect the size is in kB (kilo-byte), as supported in
    \cite{Allen_GPU_2020}.
